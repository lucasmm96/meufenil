...
| Termo | Definição no sistema | Contexto | Evidência | Relacionados |
|---|---|---|---|---|
| **Referência (alimentar)** | alimento com valor de fenilalanina por 100g (`fenil_mg_por_100g`); identidade = `(nome, marca, fenil_mg_por_100g)`, única entre referências ATIVAS (desde a ENH-0004, nome e marca são atributos separados — sem coluna `nome_normalizado`); marca não declarada = `''` EM BRANCO (canônico revisto em 2026-09-04) | tabela `referencias` | database `[CONFIRMED: database, migrations 20260904000000/20260904030000]` | Referência global, customizada, favorita, Marca |
| **Marca** | atributo separado da referência (coluna `marca`, default `''` desde 2026-09-04 — antes `'Produto In Natura'`); em branco = marca NÃO declarada; `'Produto In Natura'` é marca declarada pela fonte ANVISA (nunca imposta a produto sem marca — decisão 2026-09-04); apresentação combinada "Nome (Marca: X)" montada dinamicamente no frontend (`nomeComMarca`) só quando há marca declarada | modal de referência, lista, busca (nome OU marca) | database, code `[CONFIRMED: database, code]` | Referência (alimentar) |
...
- [overview.md](Specs-Current-Product-Overview), [../domain/domain-model.md](Specs-Current-Domain-Domain-Model), [../domain/business-rules.md](Specs-Current-Domain-Business-Rules)
