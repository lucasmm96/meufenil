...
**Última verificação:** 2026-09-11 (FEAT-0017 M1–M6 — migrations 20260905*/20260906*/20260907000000 aplicadas em dev e prod — release v1.11.0, 2026-09-10. Migration 20260911000000 (`numeric(10,1)` → `numeric(10,2)`) aplicada em dev em 2026-09-11; prod segue com `numeric(10,1)` até a aplicação — ver seção Migrations)

## Propósito

Este diretório documenta o estado REAL do banco de dados PostgreSQL (Supabase) do MeuFenil — tabelas, funções, triggers e políticas RLS — conforme observado nos bancos development e production, complementado pelas migrations versionadas e pelo código que interage com o banco. A divergência ENH-0004/FEAT-0017 M1–M6 entre dev e prod foi **encerrada pela release v1.11.0 (2026-09-10)**, que aplicou as migrations 20260904000000→20260907000000 em prod `[CONFIRMED: database — supabase_migrations.schema_migrations prod 2026-09-11]`. Desde então, dev e prod divergem apenas em: escala do fenil (`numeric(10,2)` em dev desde 2026-09-11 × `numeric(10,1)` em prod até a aplicação da 20260911000000), extensão `pg_graphql` (dev-only) e dados. As seções abaixo descrevem o estado dos dois ambientes quando não indicado.
...
### Tabelas de sincronização de referências (FEAT-0017 M1 — migration 20260905000000; dev e prod desde a release v1.11.0)

`[CONFIRMED: database — pg_tables dev 2026-09-06]`. Usadas pela rota `api/referencias-sync.ts` (M2–M4) e RPCs (M4/M5); RLS/policies em cada página e no [security-model](Specs-Current-Security-Security-Model) §11. Página própria por tabela (promoção M7, 2026-09-07).

| Tabela | Propósito curto | RLS | DDL versionado? |
|---|---|---|---|
| [`referencia_syncs`](Specs-Current-Database-Referencia_syncs) | Execuções da sincronização (status, modo, contadores, `alteracoes` jsonb) | Sim | Sim (20260905000000) |
| [`referencia_sync_pendencias`](Specs-Current-Database-Referencia_sync_pendencias) | Divergências abertas para curadoria (decisão humana por admin) | Sim | Sim (20260905000000) |
| [`referencia_eventos`](Specs-Current-Database-Referencia_eventos) | Auditoria de eventos da sync e de mudanças manuais de `is_ativa` | Sim | Sim (20260905000000; trigger de auditoria em 20260905010000) |
| [`referencia_snapshots`](Specs-Current-Database-Referencia_snapshots) | Snapshots da extração (uma por execução) | Sim | Sim (20260905000000) |
| [`referencia_backups`](Specs-Current-Database-Referencia_backups) | Backups pré-aplicação (base do rollback/restauração — M5); retenção 12m | Sim | Sim (20260905000000) |
...
- **RLS habilitado em TODAS as tabelas** — dev e prod: 12 (inclui as 5 de sync do FEAT-0017 M1; prod desde a release v1.11.0) `[CONFIRMED: database — pg_class.relrowsecurity = true; catálogo dev/prod 2026-09-11]`.
- **Grants amplos**: `anon`, `authenticated` e `service_role` possuem SELECT/INSERT/UPDATE/DELETE/TRUNCATE/REFERENCES/TRIGGER em todas as tabelas (dev: 12 — as 5 de sync nasceram com grants do default privileges do Supabase); a restrição efetiva de acesso é o RLS `[CONFIRMED: database — role_table_grants]`. Nas tabelas de sync a leitura é limitada a admin (policies `admin_select_*` — ver [security-model](Specs-Current-Security-Security-Model) §11).
...
| `20260904000000_referencias_marca_identidade_imutavel.sql` | 2026-09-04 | **ENH-0004:** coluna `marca` (default canônico `'Produto In Natura'` — REVOGADO em 2026-09-04 pela 030000, final = `''`), `nome_normalizado` eliminada, `fenil_mg_por_100g` → `numeric(10,1)`, backfill do sufixo `(Marca: ...)` do nome, triggers de normalização e de favoritos eliminados, RPC `remover_ou_desativar_referencia` redefinida (globais sempre arquivam), índice único parcial `referencias_identidade_ativa_unique` — aplicada em DEV em 2026-09-04; prod: release v1.11.0 (2026-09-10) |
| `20260904010000_referencias_marca_backfill_aninhados.sql` | 2026-09-04 | **ENH-0004 (complemento):** backfill de 4 linhas com parênteses aninhados na marca que o regex principal não capturou — aplicada em DEV em 2026-09-04; prod: release v1.11.0 (2026-09-10) |
| `20260904020000_referencias_marca_correcao_wrapper.sql` | 2026-09-04 | **Correção pós-merge (bug ENH-0004):** o backfill da 000000 usou `regexp_match` sem grupo de captura e gravou o invólucro literal `(Marca: X)` na coluna `marca`; re-extração do conteúdo em 2.955 linhas de dev (total 3.164 preservado, 0 residuais, 0 duplicatas) — aplicada em DEV em 2026-09-04; prod: release v1.11.0 (2026-09-10) |
| `20260904030000_referencias_marca_sem_marca_em_branco.sql` | 2026-09-04 | **Canônico de marca revisto (decisão do usuário 2026-09-04):** default da coluna `'Produto In Natura'` → `''`; "Em branco = Produto In Natura" revogada — sem marca declarada = `''`; `'Produto In Natura'` mantido só onde a planilha ANVISA declara in natura (97 em dev; 222 em branco; 3.164 preservado; 0 duplicatas) — aplicada em DEV em 2026-09-04; prod: release v1.11.0 (2026-09-10) |
| `20260904040000_referencias_marca_apos_nome.sql` | 2026-09-04 | **Reordenação física (decisão do usuário 2026-09-04):** `marca` imediatamente após `nome` também no schema (Postgres não tem ADD COLUMN AFTER — tabela recriada: rename → cópia com lista explícita → drop; 4 políticas cross-tabela de `registros`/`referencias_favoritas` dependiam da tabela por OID e foram dropadas/recriadas verbatim — 1ª aplicação falhou com 2BP01, corrigida e reaplicada). PK, FK, índice único parcial, RLS, 10 políticas da tabela, grants e FKs dos filhos recriados idênticos; verificação: 3.164 linhas preservadas, `marca` posição 3, segurança 28/28 — aplicada em DEV em 2026-09-04; prod: release v1.11.0 (2026-09-10) (última do trem 000000→040000, entrega prod já no layout final) |
| `20260905000000_referencias_sync_tabelas.sql` | 2026-09-06 | **FEAT-0017 M1:** 5 tabelas de sincronização (`referencia_syncs`, `referencia_sync_pendencias`, `referencia_eventos`, `referencia_snapshots`, `referencia_backups`), 4 enums (`sync_status`, `sync_pendencia_status`, `sync_pendencia_tipo`, `sync_evento_tipo`), função/trigger `trg_trim_referencia_backups` (retenção 12 meses) e 5 policies `admin_select_*` — aplicada em DEV em 2026-09-06; prod: release v1.11.0 (2026-09-10) |
| `20260905010000_referencias_sync_auditoria_is_ativa.sql` | 2026-09-06 | **FEAT-0017 M1:** função/trigger `trg_auditar_is_ativa_manual` — auditoria da mudança MANUAL de `is_ativa` em `referencias` (WHEN `auth.uid()` não-nulo; registra só admin com GUC `app.audit_origin` ≠ `'curadoria'` — escritas automáticas de sync M4/M5 não geram o evento) — aplicada em DEV em 2026-09-06; prod: release v1.11.0 (2026-09-10) |
| `20260905020000_referencias_ativar_global_somente_admin.sql` | 2026-09-06 | **FEAT-0017 M1 (R4-3):** `ativar_referencia` redefinida — reativação de referência GLOBAL passa a exigir admin (usuário comum só reativa pessoais) — aplicada em DEV em 2026-09-06; prod: release v1.11.0 (2026-09-10) |
| `20260906000000_referencias_sync_aplicacao_curadoria.sql` | 2026-09-06 | **FEAT-0017 M4:** RPCs `aplicar_sync_referencias` (SECURITY DEFINER; EXECUTE somente `service_role`) e `decidir_pendencia_referencia` (SECURITY DEFINER; EXECUTE `authenticated` com guarda interna de admin) — aplicada em DEV em 2026-09-06; prod: release v1.11.0 (2026-09-10) |
| `20260906010000_referencias_sync_rollback_restauracao.sql` | 2026-09-06 | **FEAT-0017 M5:** coluna `usuarios.pode_recuperacao` (default `false` — concedida manualmente pelo dono, nenhum fluxo de app auto-concede), helper `pode_operar_recuperacao` (admin E flag, formato de `is_admin_user`) e RPCs SECURITY DEFINER `reverter_sync_referencias` (rollback seletivo — inversas das `alteracoes` em ordem reversa com guarda por op) e `restaurar_referencias_de_backup` (restauração excepcional — conjunto global volta a refletir o backup; integridade sha256 verificada antes; nunca DELETE; não toca pessoais) — EXECUTE **somente `authenticated`** (guarda interna `pode_operar_recuperacao`) — aplicada em DEV em 2026-09-06; prod: release v1.11.0 (2026-09-10) |
| `20260907000000_referencias_sync_seed_pre_sync_inativa.sql` | 2026-09-07 | **FEAT-0017 M6:** `CREATE OR REPLACE` de `aplicar_sync_referencias` com passo de seed condicionado a `p_plano.modo = 'bootstrap'` (1ª sync confiável do ambiente): 1 evento `pre_sync_inativa` por global inativa SEM evento de auditoria (actor NULL — arquivamento manual pré-auditoria), na MESMA transação da aplicação; nenhum objeto novo — aplicada em DEV em 2026-09-07; prod: release v1.11.0 (2026-09-10) |
| `20260911000000_referencias_fenil_numeric_10_2.sql` | 2026-09-11 | **FEAT-0017 (decimal de fenilalanina):** `referencias.fenil_mg_por_100g` → `numeric(10,2)` (ampliação exata — a origem entrega até 2 casas e a escala 1 arredondava na escrita) e `CREATE OR REPLACE` das 4 RPCs que carregavam o cast `numeric(10,1)`: `aplicar_sync_referencias`, `decidir_pendencia_referencia`, `reverter_sync_referencias`, `restaurar_referencias_de_backup` (corpo idêntico, só o tipo muda — inclusive as comparações de igualdade do rollback, que com escala 1 tratariam 5,42 como 5,4). Nenhum objeto novo; sem alteração de assinaturas, ACLs, políticas ou índices (o índice único parcial é reconstruído automaticamente pelo `ALTER COLUMN`). **Aplicada em DEV em 2026-09-11; prod segue `numeric(10,1)` até a aplicação** (aplicação é passo separado, com autorização) `[CONFIRMED: migration; database — catálogo dev 2026-09-11]` |
| `20260914000000_reverter_sync_sem_ops_cancela_pendencias.sql` | 2026-09-14 | **FEAT-0017 (revisão do reverter — decisão do usuário):** `CREATE OR REPLACE` de `reverter_sync_referencias` — sync sem alterações COM pendências `open` (caso bootstrap) cancela as pendências (evento `pendencia_cancelada` por pendência) e marca a sync `reverted`; sem alterações E sem pendências mantém o no-op informativo. Nenhum objeto novo; sem alteração de assinaturas, ACLs, grants ou policies — aplicada em DEV em 2026-09-14; prod aguarda a release que a levar `[CONFIRMED: migration; database — catálogo dev 2026-09-14]` |
...
- **ENH-0004 (dev 2026-09-04 → prod release v1.11.0, 2026-09-10)**: migrations 20260904000000/010000/020000/030000/040000 aplicadas em DEV em 2026-09-04 e em PROD na release v1.11.0. No estado intermediário pós-ENH-0004 em dev: **14 índices** (15 − 2 únicos de nome + 1 `referencias_identidade_ativa_unique`), **8 funções** (10 − `fn_normalizar_nome_referencia` − `fn_remover_favoritos_referencia_inativa`), **1 trigger em `public`** (3 − `trg_normalizar_nome_referencia` − `trg_remover_favoritos_referencia_inativa`) + 1 em `auth.users` (inalterado); colunas seguem 52 (nome_normalizado removida, marca adicionada), 19 constraints e 31 políticas inalterados (a 040000 recriou as políticas verbatim — sem mudança de contagem). A divergência foi encerrada pela release — prod hoje tem o mesmo modelo canônico (ver itens seguintes).
- **FEAT-0017 M1–M6 (dev 2026-09-06/07 → prod release v1.11.0, 2026-09-10)**: migrations 20260905000000/010000/020000 (M1), 20260906000000 (M4), 20260906010000 (M5 — rollback/restauração) e 20260907000000 (M6) aplicadas em DEV em 2026-09-06/07 e em PROD na release v1.11.0. Os dois ambientes têm hoje `[CONFIRMED: database — catálogo dev/prod 2026-09-11]`: **12 tabelas** (7 + 5 de sync), **101 colunas** (52 + 48 + `usuarios.pode_recuperacao` do M5), **36 constraints** (`pg_constraint`: 12 PK + 19 FK + 1 UNIQUE + 4 CHECK), **36 índices**, **36 políticas** (31 + 5 `admin_select_*`), **15 funções** (8 pós-ENH-0004 + 2 de trigger do M1 + 2 RPCs do M4 + 3 do M5), **5 enums** (1 legado + 4 de sync) e **3 triggers em `public`** (1 + 2 do M1) + 1 em `auth.users`. O M6 (20260907000000) é `CREATE OR REPLACE` de função — **nenhum objeto novo** (contagens do M5 permanecem).
- **Diferenças entre ambientes**:
  1. Extensão `pg_graphql` presente em dev, ausente em prod `[CONFIRMED: database]`.
  2. **Resolvido na release v1.11.0:** prod possuía uma coluna **dropped** na posição física 8 de `referencias` (artefato de ADD+DROP; o nome original não é recuperável do catálogo — `UNKNOWN`) `[CONFIRMED: database — pg_attribute.attisdropped prod, medição histórica]`. A migration 20260904040000 rodou em prod no trem de release e eliminou a coluna (tabela recriada a partir das colunas nomeadas no INSERT) — hoje **0 colunas dropped em prod** `[CONFIRMED: database — pg_attribute.attisdropped prod 2026-09-11]`.
  3. Schema de `referencias`: **dev e prod** têm `marca` (com **ordem física logo após `nome`** — prod desde a release v1.11.0, 2026-09-10) e não têm `nome_normalizado` nem os 2 triggers da ENH-0004 `[CONFIRMED: migrations 20260904000000→040000 — execução dev 2026-09-04 e prod 2026-09-10]`. **Escala do fenil:** dev tem `numeric(10,2)` desde a migration `20260911000000` (aplicada em dev em 2026-09-11); prod tem `numeric(10,1)` (recebido da `20260904000000` na release) até a aplicação da `20260911000000` em prod `[CONFIRMED: database — catálogo dev/prod 2026-09-11]`.
...
- E2 — Conteúdo das migrations em `supabase/migrations/` (baseline, 20260807, 20260810, 20260811, 20260814, 20260815, 20260904000000→040000, FEAT-0017: 20260905000000/010000/020000, 20260906000000, 20260906010000, 20260907000000 e 20260911000000) `[CONFIRMED: migration]`.
...
- [system-map](Specs-Current-System-Map) — coluna Database do mapa de capabilities
