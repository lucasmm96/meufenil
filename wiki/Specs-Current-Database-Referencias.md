...
- **É referenciada por:**
  - `registros.referencia_id` → `referencias(id)` (sem CASCADE — ver [registros.md](Specs-Current-Database-Registros))
...
- O DELETE direto por usuário é bloqueado quando existem `registros` vinculados (o vínculo é preservado — ver [registros.md](Specs-Current-Database-Registros)); a via oficial de remoção é o RPC `remover_ou_desativar_referencia` — para GLOBAIS o RPC sempre arquiva (`is_ativa = false`), nunca exclui fisicamente (OQ4 da ENH-0004); para pessoais mantém a regra soft/hard conforme vínculo `[CONFIRMED: database, migration — 20260904000000]`.
...
- Desativação (arquivamento): `is_ativa = false`; NÃO remove favoritos em nenhum fluxo (trigger de remoção de favoritos foi ELIMINADO na ENH-0004 — OQ3/BR-036; ver [triggers.md](Specs-Current-Database-Triggers) e [referencias_favoritas.md](Specs-Current-Database-Referencias_favoritas)).
...
- Novos registros de consumo exigem referência ativa (política de `registros` — ver [registros.md](Specs-Current-Database-Registros)).
...
- `ativar_referencia` (UPDATE `is_ativa = true`) — [rpc.md](Specs-Current-Database-Rpc)
- `remover_ou_desativar_referencia` (SELECT/DELETE/UPDATE; redefinida na migration 20260904000000 — globais sempre arquivam) — [rpc.md](Specs-Current-Database-Rpc)
- `aplicar_sync_referencias` (INSERT de globais com `criado_por` = Sistema; UPDATE `is_ativa = false` por ausência — FEAT-0017 M4) — [rpc.md](Specs-Current-Database-Rpc)
- `decidir_pendencia_referencia` (curadoria: UPDATE `is_ativa = false` em absence/substitution; INSERT da proposta em new_item/substitution — FEAT-0017 M4) — [rpc.md](Specs-Current-Database-Rpc)
- `reverter_sync_referencias` (rollback seletivo de sync: arquiva criações e reativa arquivamentos com guarda por op — FEAT-0017 M5) — [rpc.md](Specs-Current-Database-Rpc)
- `restaurar_referencias_de_backup` (restauração excepcional: o conjunto global volta a refletir o backup — reativa/cria/arquiva; FEAT-0017 M5) — [rpc.md](Specs-Current-Database-Rpc)
- `get_estatisticas_admin` (contagens) — [rpc.md](Specs-Current-Database-Rpc)
- Triggers: `trg_auditar_is_ativa_manual` (AFTER UPDATE OF `is_ativa`, FOR EACH ROW, `WHEN (auth.uid() is not null)` — registra `is_ativa_manual` com `{de, para}` quando o autor é admin e o GUC `app.audit_origin` ≠ `'curadoria'`; FEAT-0017 M1/D-7, suprimido nas escritas de sync do M4/M5). Desde a ENH-0004, `trg_normalizar_nome_referencia` e `trg_remover_favoritos_referencia_inativa` foram ELIMINADOS — [triggers.md](Specs-Current-Database-Triggers)
...
- [rpc.md](Specs-Current-Database-Rpc), [triggers.md](Specs-Current-Database-Triggers), [referencias_favoritas.md](Specs-Current-Database-Referencias_favoritas), [registros.md](Specs-Current-Database-Registros)
