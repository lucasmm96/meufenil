...
| FEAT-0017 | Sincronização de referências (ANVISA/Power BI) | implementada | Mecanismo recorrente, controlado e auditável de sincronização do conjunto global de referências com a origem (relatório Power BI associado à ANVISA): extração, validação, snapshot/backup por execução, aplicação automática de mudanças seguras (cron semanal Vercel) e curadoria humana de divergências no painel admin, com rollback seletivo e restauração excepcional auditados. |
