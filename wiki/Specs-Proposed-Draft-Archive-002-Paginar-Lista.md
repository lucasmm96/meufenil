# Draft 002 — refinado pelo spec-assistant (2026-08-22; OQs resolvidas em 2026-08-22)

> **Status:** DRAFT — não é proposta formal. ID sugerido: **ENH-0003** (próximo disponível em `enhancements/`; existem ENH-0001 e ENH-0002). Quando aprovado o conteúdo, o fluxo orquestrado cria `proposed/enhancements/ENH-0003-*.md` + Issue + item no Project.
> **Origem:** solicitação do usuário em linguagem natural (texto original preservado no rodapé).
> **Decisões do usuário incorporadas (2026-08-22):** (1) seletor de page size 3/10/20 com default 3 · (2) paginação numerada + Anterior/Próxima · (3) respeitar filtros vigentes · (4) escopo restrito ao "Histórico das execuções" do Admin · (5) sem persistência de page size entre sessões (default 3 a cada carregamento). Detalhes e racional nas seções Open Questions e Alternatives.
...
- A página Admin (`src/react-app/pages/Admin.tsx`) possui a seção "Monitoramento de Jobs" com filtros (Job/Status/Período) e o "Histórico das execuções" `[CONFIRMED: code — Admin.tsx:319-590]`. Spec: [`current/frontend/pages/admin.md`](Specs-Proposed-Current-Frontend-Pages-Admin) e [`current/features/FEAT-0012-painel-administrativo.md`](Specs-Proposed-Current-Features-FEAT-0012-Painel-Administrativo).
...
- Nota: o draft original pressupunha ausência de paginação; a paginação (server-side, 20/página) já existe desde antes da última verificação da spec (2026-08-13). O pedido real é reduzir/enxugar a visão inicial, dar controle do tamanho da página e melhorar a navegação — não criar paginação do zero.
...
- **FACTUAL:** o default atual é 20 execuções por página `[CONFIRMED: code — background-jobs.service.ts:26]`; o usuário pediu visão inicial de 3 (draft 002, 2026-08-22).
...
- Origem: solicitação do usuário (draft `proposed/draft/002-paginar-lista.md`, 2026-08-22) + respostas às Open Questions (2026-08-22).
...
- [`FEAT-0012 Painel administrativo`](Specs-Proposed-Current-Features-FEAT-0012-Painel-Administrativo) (única capability afetada).
...
- Draft original (texto cru do usuário): preservado abaixo.
...
### Texto original do draft (preservado)
