...
| `pode_recuperacao` | boolean | `false` | NO | — | FEAT-0017 M5 (20260906010000 — dev e prod desde a release v1.11.0): flag de recuperação de sync — NÃO é papel nem privilégio amplo; admin **E** flag habilitam `reverter_sync_referencias`/`restaurar_referencias_de_backup` (helper `pode_operar_recuperacao`). Default `false`; concedida MANUALMENTE pelo dono do projeto (nenhum fluxo de app auto-concede) |
...
- Papel de administrador = `usuarios.role = 'admin'` (implementado em `is_admin_user` — ver [rpc.md](Specs-Current-Database-Rpc)).
- Limite diário: o default da COLUNA é `500`; o trigger `handle_new_user` não define limite no sign-up — o default vale para todo novo usuário (DEBT-0002, decisão B) (ver [triggers.md](Specs-Current-Database-Triggers)).
- Delegação de acesso referencia esta tabela em `delegacoes_acesso` (ver spec própria e `../security/security-model.md` — Fase 3).
- Recuperação de sync (FEAT-0017 M5): operar `reverter_sync_referencias`/`restaurar_referencias_de_backup` exige `role = 'admin'` **E** `pode_recuperacao = true` (helper `pode_operar_recuperacao` — ver [rpc.md](Specs-Current-Database-Rpc)). A flag sozinha (usuário comum) não habilita nada; admin **sem** a flag também não opera recuperação — é permissão específica, não decorre de ser admin.
...
- `handle_new_user` (INSERT) — [rpc.md](Specs-Current-Database-Rpc)
- `is_admin_user` (SELECT sobre `role`) — [rpc.md](Specs-Current-Database-Rpc)
- `pode_operar_recuperacao` (SELECT sobre `role` + `pode_recuperacao` — FEAT-0017 M5) — [rpc.md](Specs-Current-Database-Rpc)
- `dashboard_hoje` (SELECT sobre `limite_diario_mg`) — [rpc.md](Specs-Current-Database-Rpc)
...
- E6 — Coluna `pode_recuperacao` + helper `pode_operar_recuperacao`: migration 20260906010000 (FEAT-0017 M5 — aplicada em dev 2026-09-06 e em prod na release v1.11.0, 2026-09-10) `[CONFIRMED: migration, database]`

## Veja também

- [rpc.md](Specs-Current-Database-Rpc), [triggers.md](Specs-Current-Database-Triggers), [delegacoes_acesso.md](Specs-Current-Database-Delegacoes_acesso)
