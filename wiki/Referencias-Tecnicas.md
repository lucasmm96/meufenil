...
- **Rota:** `api/referencias-sync.ts` (Vercel) — GET (cron semanal `0 12 * * 1` UTC) exige `Authorization: Bearer ${CRON_SECRET}` (comparação timing-safe); POST manual exige JWT de admin (assina `requested_by`). Envs: `REFERENCIAS_SYNC_SUPABASE_URL` / `REFERENCIAS_SYNC_SUPABASE_SERVICE_ROLE_KEY` (dedicadas, obrigatórias, sem fallback), `CRON_SECRET`, `POWERBI_RESOURCE_KEY`. Alvo: **somente prod**. (Fonte: `backend/api-referencias-sync.md`)
