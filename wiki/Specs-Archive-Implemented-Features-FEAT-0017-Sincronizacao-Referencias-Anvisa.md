...
- [FEAT-0012 Painel administrativo](Specs-Archive-Implemented-Current-Features-FEAT-0012-Painel-Administrativo) — novas seções de sincronizações/curadoria/histórico/diff/rollback/auditoria (padrão ENH-0003: filtros, paginação, tabela×cards, modal de detalhes)
- [FEAT-0013 Background jobs](Specs-Archive-Implemented-Current-Features-FEAT-0013-Background-Jobs) — infraestrutura de jobs reaproveitada/estendida (B2/B6)
- [FEAT-0008 Referências alimentares](Specs-Archive-Implemented-Current-Features-FEAT-0008-Referencias-Alimentares) — interação com lifecycle/arquivamento e indicadores de estado (conjunto global)
- [ENH-0003 Histórico das execuções](Specs-Archive-Implemented-Archive-Implemented-Enhancements-ENH-0003-Historico-Execucoes-Seletor-Paginacao) — precedente de UI reaproveitado (arquivada — referência de padrão)

## Impacted Business Rules

Existentes (links — as que o sync deve respeitar/estender):
- [BR-017](Specs-Archive-Implemented-Current-Domain-Business-Rules) — remoção de global exclusiva de admin
- [BR-018](Specs-Archive-Implemented-Current-Domain-Business-Rules) — referência com registros não é excluída hard
- [BR-019](Specs-Archive-Implemented-Current-Domain-Business-Rules) — registro exige referência ativa
- [BR-023](Specs-Archive-Implemented-Current-Domain-Business-Rules) — edição/remoção na UI (dono OU admin E global)
- [BR-024](Specs-Archive-Implemented-Current-Domain-Business-Rules) — lifecycle ativa ↔ inativa (sync nunca reativa; favoritos preservados após ENH-0004)
- [BR-026](Specs-Archive-Implemented-Current-Domain-Business-Rules) — exclusão de conta (interação com linhas `criado_por = Sistema` — B5)
- [BR-027](Specs-Archive-Implemented-Current-Domain-Business-Rules) — retenção 365d de `background_job_executions` (colide com histórico longo de sync — B6)
...
- [ADR-0007 — Keepalive/cron jobs](Specs-Archive-Decisions-ADR-0007-Keepalive-Cron-Jobs) — novo portador de agendamento (B2) integrado à infra Vercel/Supabase existente
- [ADR-0010 — RPCs SECURITY DEFINER](Specs-Archive-Decisions-ADR-0010-Rpcs-Security-Definer) — novas funções/escritas seguem o padrão (B5/B6)
- [ADR-0004 — RLS como enforcement](Specs-Archive-Decisions-ADR-0004-Rls-Como-Enforcement) — autorização das novas tabelas/operações (B6/B7/B8)
- [overview](Specs-Archive-Current-Architecture-Overview) — sem servidor de aplicação; execução em Vercel/Supabase conforme decisões B1/B2
...
- **Frontend:** [pages/admin.md](Specs-Archive-Implemented-Current-Frontend-Pages-Admin), `useBackgroundJobsAdmin`/`background-jobs.service` e componentes de histórico — novas seções de sincronizações, curadoria (aprovar/rejeitar + motivo), diff e auditoria; DTOs novos; padrões visuais de FEAT-0012/ENH-0003 (filtros, paginação, modal de mensagem).
- **Backend:** `api/` (nova rota/portador do cron — B2), `src/shared/background-jobs.ts` (reuso/extensão para `job_key='referencias-sync'` — B6), integração com o `powerbi-export` (B1); secrets/ambiente conforme [secrets-and-environments.md](Specs-Archive-Implemented-Current-Security-Secrets-And-Environments).
...
- **Security:** ator "Sistema" com identidade real no Supabase Auth (B5); permissão específica para rollback/recuperação estendendo `is_admin_user`/helpers — sem sistema paralelo (rel.: FEAT-0015, SEC-0001); proteção de snapshots/backups (B3); auditoria de operações destrutivas; [security-model](Specs-Archive-Implemented-Current-Security-Security-Model) atualizado na promoção.
- **Tests:** novas suítes de extração/validação, matching determinístico, comparação bidirecional, curadoria, rollback/restauração, RLS das novas tabelas, idempotência, concorrência (quando viável) e bootstrap; revisão dos testes existentes de `referencias`/favoritos/RPCs (draft §47); relacionadas: [TEST-0002](Specs-Archive-Implemented-Testing-TEST-0002-Suites-Seguranca-Policies), [TEST-0003](Specs-Archive-Implemented-Testing-TEST-0003-Testes-Server-Side).

## Dependencies

- **ENH-0004 — Modelo canônico e identidade imutável de referências** ([arquivada — IMPLEMENTED](Specs-Archive-Archive-Implemented-Enhancements-ENH-0004-Modelo-Identidade-Referencias)) — esta proposta depende do modelo canônico (coluna `marca`, identidade `nome+marca+fenil`, índices) e da preservação de favoritos; ENH-0004 foi implementada primeiro (PR #55, merge `82bd0f3`, 2026-09-04) e entregou essa base — ordem definida em 2026-09-02.
- Relacionados (não duplicados): [FEAT-0015](Specs-Archive-Implemented-Features-FEAT-0015-Atribuicao-Papel-Admin) (gestão de papel admin), [SEC-0001](Specs-Archive-Implemented-Security-SEC-0001-Autorizacao-Funcoes-Consulta) (autorização de funções de consulta), [REF-0002](Specs-Archive-Implemented-Refactors-REF-0002-Rpcs-Orfas-Dashboard) (destino de RPCs órfãs), [TEST-0002](Specs-Archive-Implemented-Testing-TEST-0002-Suites-Seguranca-Policies) e [TEST-0003](Specs-Archive-Implemented-Testing-TEST-0003-Testes-Server-Side) (cobertura de segurança/server-side).
...
- Draft `001-auto-refresh-database.md` (arquivado): [../draft/archive/001-auto-refresh-database.md](Specs-Archive-Implemented-Draft-Archive-001-Auto-Refresh-Database)
