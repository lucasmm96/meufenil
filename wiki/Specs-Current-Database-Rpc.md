...
**Última verificação:** 2026-09-11 (FEAT-0017 M4/M5 — migrations 20260906000000/20260906010000 aplicadas em dev e prod — release v1.11.0, 2026-09-10; grants conferidos no catálogo dev/prod 2026-09-11)

Inventário das **15 funções** do schema `public` no estado pós-FEAT-0017 M1–M5 em dev (2026-09-06) `[CONFIRMED: catálogo dev — pg_proc 2026-09-06]`. As funções de trigger `fn_normalizar_nome_referencia()` e `fn_remover_favoritos_referencia_inativa()` foram **eliminadas** pela ENH-0004 (normalização armazenada — A4(b); remoção de favoritos ao desativar — OQ3) — eliminadas também em prod na release v1.11.0. O FEAT-0017 acrescentou 7 funções em dev: 2 de trigger no M1 (`fn_auditar_is_ativa_manual`, `fn_trim_referencia_backups` — seções abaixo, com semântica do trigger em [triggers.md](Specs-Current-Database-Triggers)), as 2 RPCs do M4 (`aplicar_sync_referencias`, `decidir_pendencia_referencia`) e, no M5 (`20260906010000`), o helper `pode_operar_recuperacao` + as 2 RPCs de recuperação `reverter_sync_referencias`/`restaurar_referencias_de_backup` (rollback seletivo de sync e restauração excepcional por backup — seções abaixo). Prod tem as **mesmas 15 funções** do dev desde a release v1.11.0 (2026-09-10) `[CONFIRMED: database — catálogo prod 2026-09-11]`. Não há outras funções em `public` além das listadas `[CONFIRMED: catálogo dev — pg_proc 2026-09-06]`.
...
**Definição em:** `20260906000000_referencias_sync_aplicacao_curadoria.sql` (linhas 33–207; REVOKE/GRANT nas linhas 459–460) — aplicada em dev em 2026-09-06 e em prod na release v1.11.0 (2026-09-10) `[CONFIRMED: migration, database]`

- **Assinatura:** `aplicar_sync_referencias(p_sync_id uuid, p_plano jsonb) RETURNS jsonb` — plpgsql
- **SECURITY DEFINER?** Sim — `SET search_path TO 'public'`
- **Autorização implementada (FEAT-0017 §7.5):** guarda interna `auth.role() <> 'service_role'` → `RAISE EXCEPTION 'Permissão negada: apenas service_role pode aplicar o plano de sync'` (barreira a desvios de definer). **ACL (fato do catálogo 2026-09-11):** o `REVOKE ... FROM PUBLIC` removeu o grant a PUBLIC, mas `anon`/`authenticated`/`service_role` mantêm EXECUTE nominal via default privileges do Supabase — a proteção efetiva é a guarda interna, não a ACL. Nenhum papel de cliente (anon/authenticated) pode aplicar plano — nem admin: aplicação é sempre automática (rota/scripts)
...
**Última verificação:** 2026-09-06 (FEAT-0017 M4 — migration 20260906000000 aplicada em dev)
**Definição em:** `20260906000000_referencias_sync_aplicacao_curadoria.sql` (linhas 226–450; REVOKE/GRANT nas linhas 462–463) — aplicada em dev em 2026-09-06 e em prod na release v1.11.0 (2026-09-10) `[CONFIRMED: migration, database]`
...
- **SECURITY DEFINER?** Sim — `SET search_path TO 'public'`
- **Autorização implementada (FEAT-0017 §8):** guarda interna `NOT is_admin_user(auth.uid())` → `RAISE EXCEPTION 'Permissão negada: apenas administradores podem decidir pendências'`. **ACL (fato do catálogo 2026-09-11):** `REVOKE ... FROM PUBLIC` removeu o grant a PUBLIC, mas as roles mantêm EXECUTE nominal via default privileges — a proteção efetiva é a guarda interna. **service_role fora** — curadoria é ação humana de admin com sessão (testado)
...
**Definição em:** `20260906010000_referencias_sync_rollback_restauracao.sql` (linhas 103–317; REVOKE/GRANT nas linhas 617–618) — aplicada em dev em 2026-09-06 e em prod na release v1.11.0 (2026-09-10); **revisada** pela migration `20260914000000_reverter_sync_sem_ops_cancela_pendencias.sql` (no-op com pendências open → cancela + reverted; decisão do usuário 2026-09-14) — aplicada em dev em 2026-09-14 `[CONFIRMED: migration, database]`

- **Assinatura:** `reverter_sync_referencias(p_sync_id uuid) RETURNS jsonb` — plpgsql
- **SECURITY DEFINER?** Sim — `SET search_path TO 'public'`
- **Autorização implementada (FEAT-0017 §10.1/§12.4):** guarda interna `NOT pode_operar_recuperacao(auth.uid())` → `RAISE EXCEPTION 'Permissão negada: apenas administradores com permissão de recuperação (usuarios.pode_recuperacao) podem reverter syncs'`. **ACL (fato do catálogo 2026-09-11):** `REVOKE ... FROM PUBLIC` removeu o grant a PUBLIC, mas as roles mantêm EXECUTE nominal via default privileges — a proteção efetiva é a guarda interna. **service_role fora** — rollback é ação humana excepcional de admin com a flag (nunca automatizada)
- **Pré-condições e validação:** sync deve existir (`'Sync não encontrada: %'`); guarda de serialização B10c **por environment do alvo** (decisão humana 2026-09-06) — sync `running` no MESMO environment bloqueia (`'Existe sync em execução neste environment — aguarde a conclusão antes de reverter'`; outras environments não bloqueiam; a própria sync `running` cai na guarda de status abaixo); lock `FOR UPDATE` das pendências `open` do alvo ANTES do lock da sync (ordem pendências → syncs espelha `decidir_pendencia_referencia` e evita deadlock com decisão concorrente); estado revalidado sob lock — status deve ser `success`/`pending_review` (`'Sync não pode ser revertida (status = % — apenas success/pending_review)'`). **No-op (revisado 2026-09-14 — migration 20260914000000):** sync sem `alteracoes` **E** sem pendências `open` → retorno `{sync_id, status, revertida: false, motivo}` SEM mudança de estado. Sync sem `alteracoes` **COM** pendências `open` (caso bootstrap — bootstrap por definição não aplica operações) → as pendências são **canceladas** (evento `pendencia_cancelada` por pendência) e a sync marcada `reverted` com `0 operação(ões) revertida(s), N pendência(s) cancelada(s)` — decisão do usuário: sem isso, pendências de uma sync bootstrap (ex.: retrato truncado de catálogo) jamais poderiam ser descartadas em lote, e a sync seguiria contando como confiável na derivação de modo
...
- **Chamadores no código:** `src/react-app/services/referencias-sync.service.ts:555` — `.rpc("reverter_sync_referencias", ...)` com sessão de admin (UI de recuperação do Admin, M6); ação humana excepcional, nenhum chamador automático (a rota não chama; a guarda interna exclui service_role) `[CONFIRMED: code — referencias-sync.service.ts; migration]`
- **Testes:** suíte REAL `src/shared/security/rpc-referencias-sync-rollback.test.ts` (18 testes — guards `isFeat0017M5Applied`/`isFeat0017ReverterSemOpsRevisado`/admins com e sem a flag): permissão negada (admin sem flag; não-admin), sync inexistente, happy path (ordem reversa; eventos `rollback` por op com `{operacao, inversa, resultado}`; reativação SEM `is_ativa_manual`), no-op sem alterações E sem pendências, **sem alterações COM pendências open → cancela + reverted** (revisão 2026-09-14), skips (já arquivada; reativada manualmente depois — alteração posterior preservada; `23505` → skip por op), cancela **apenas open** do alvo (decididas e de outra sync intactas), status não-revertível, guarda running do mesmo environment × outro environment não bloqueia `[CONFIRMED: test]`
...
**Última verificação:** 2026-09-06 (FEAT-0017 M5 — migration 20260906010000 aplicada em dev)
**Definição em:** `20260906010000_referencias_sync_rollback_restauracao.sql` (linhas 345–608; REVOKE/GRANT nas linhas 620–621) — aplicada em dev em 2026-09-06 e em prod na release v1.11.0 (2026-09-10) `[CONFIRMED: migration, database]`
...
- **Chamadores no código:** `src/react-app/services/referencias-sync.service.ts:574` — `.rpc("restaurar_referencias_de_backup", ...)` com sessão de admin (UI de recuperação do Admin, M6); ação humana excepcional, nenhum chamador automático (a rota não chama; a guarda interna exclui service_role) `[CONFIRMED: code — referencias-sync.service.ts; migration]`
