...
- A autorização EFETIVA é do banco (RLS/RPCs) — ver [../security/security-model.md](Specs-Current-Security-Security-Model). A UI implementa apenas visibilidade/experiência `[CONFIRMED: security-model]`.
...
- **Números:** `toFixed(1)` para valores em mg (exceção: a coluna `fenil_mg_por_100g` é renderizada com `toFixed(2)` — escala `numeric(10,2)` desde a migration 20260911000000), `text-2xl sm:text-3xl font-bold`; cores semânticas verde/vermelho/âmbar por estado.
...
- [../backend/overview.md](Specs-Current-Backend-Overview) (frontend × backend), [../security/security-model.md](Specs-Current-Security-Security-Model) (auth/UI authorization), [../database/](Specs-Current-Database) (tabelas consumidas)
