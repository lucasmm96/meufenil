...
- Infraestrutura documentada em [api-keepalive.md](Specs-Archive-Current-Backend-Api-Keepalive), [background-jobs.md](Specs-Archive-Current-Backend-Background-Jobs), [FEAT-0013](Specs-Archive-Current-Features-FEAT-0013-Background-Jobs) e [ADR-0007](Specs-Archive-Decisions-ADR-0007-Keepalive-Cron-Jobs).
...
- Documentação atual documenta "1 alvo por execução" como comportamento vigente (README corrigido pelo [DEBT-0003](Specs-Archive-Archive-Implemented-Technical-Debt-DEBT-0003-Atualizar-Readme) e `api-keepalive.md`) — ou seja, codifica a regressão como especificação `[CONFIRMED: docs — README.md, current/backend/api-keepalive.md]`.
...
- [FEAT-0013 — Background jobs](Specs-Archive-Current-Features-FEAT-0013-Background-Jobs)
...
- [ADR-0007 — Keepalive diário via Vercel Cron](Specs-Archive-Decisions-ADR-0007-Keepalive-Cron-Jobs) (Consequence 1 — "um alvo por execução" — será revisitada caso a proposta seja aceita)
- [Architecture overview](Specs-Archive-Current-Architecture-Overview) (infra Vercel)

## Impacted Frontend / Backend / Database / Security / Tests

- **Backend:** `api/keepalive.ts`, `src/shared/background-jobs.ts` (helper reutilizável — sem mudança prevista) — [background-jobs.md](Specs-Archive-Current-Backend-Background-Jobs), [api-keepalive.md](Specs-Archive-Current-Backend-Api-Keepalive)
- **Security:** env vars dev voltam a ser alvo real de execução — [secrets-and-environments.md](Specs-Archive-Current-Security-Secrets-And-Environments) §2 e §5
...
- [ADR-0007 — Keepalive diário via Vercel Cron](Specs-Archive-Decisions-ADR-0007-Keepalive-Cron-Jobs)
