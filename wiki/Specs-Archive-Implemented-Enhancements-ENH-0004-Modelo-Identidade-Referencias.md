...
**Decision revision 3 (2026-09-11, pós-FEAT-0017):** a decisão A2 (`numeric(10,1)` — "comporta 1 casa decimal") foi **superada** pelo FEAT-0017: a origem ANVISA/Power BI entrega valores de fenilalanina com até 2 casas decimais e a escala 1 arredondava na escrita. Decisão firmada pelo usuário em 2026-09-11 (valor físico decimal é válido; nunca arredondar o valor da origem; limite de 2 casas; rejeição com motivo distinto para ">2 casas" × "não numérico"). Implementada pela migration `20260911000000` (`referencias.fenil_mg_por_100g` → `numeric(10,2)` + `CREATE OR REPLACE` das 4 RPCs de sync que carregavam o cast `numeric(10,1)` — corpos idênticos, só o tipo muda), aplicada em dev em 2026-09-11; pendente em prod.
...
- [FEAT-0008 Referências alimentares](Specs-Archive-Implemented-Current-Features-FEAT-0008-Referencias-Alimentares) — criação/edição/exibição, favoritos, desativação/reativação, mensagens de erro (inclui `REFERENCIA_DUPLICADA`, hoje "Já existe uma referência com esse nome.")
- [FEAT-0003 Registro diário de consumo](Specs-Archive-Implemented-Current-Features-FEAT-0003-Registro-Diario-Consumo) — criação inline de referência a partir do modal de registro

## Impacted Business Rules

Existentes (links):
- [BR-018](Specs-Archive-Implemented-Current-Domain-Business-Rules) — soft × hard delete (OQ4 resolvida: globais nunca hard delete pela aplicação; pessoais sem registros mantêm o fluxo atual)
- [BR-023](Specs-Archive-Implemented-Current-Domain-Business-Rules) — edição/remoção na UI: dono OU (admin E global) — edição substantiva de **globais** deixa de ser UPDATE in-place (OQ1); edição substantiva de pessoais pelo dono permanece (sem mudança)
- [BR-024](Specs-Archive-Implemented-Current-Domain-Business-Rules) — lifecycle criada → ativa ↔ inativa: preservação de favoritos em qualquer desativação (OQ3) e reativação manual de pessoais mantida; fronteira "arquivada não reativa" restrita ao conjunto sincronizado (FEAT-0017)
...
- [ADR-0006 — Soft delete de referências](Specs-Archive-Decisions-ADR-0006-Soft-Delete-Referencias) — a política de arquivamento evolui (reativação — OQ3; exclusão física — OQ4)
- [ADR-0009 — Migrations via Supabase CLI](Specs-Archive-Decisions-ADR-0009-Migrations-Supabase-Cli) — migração versionada pelo mecanismo oficial (`scripts/apply-supabase-migrations.sh`)
- [overview](Specs-Archive-Current-Architecture-Overview) — sem mudança de camadas

## Impacted Frontend / Backend / Database / Security / Tests

- **Frontend:** [pages/referencias.md](Specs-Archive-Implemented-Current-Frontend-Pages-Referencias), [components/modal-referencia.md](Specs-Archive-Implemented-Current-Frontend-Components-Modal-Referencia), [components/adicionar-registro.md](Specs-Archive-Implemented-Current-Frontend-Components-Adicionar-Registro) e `referencias.service`/DTOs — campos `nome` + `marca` separados nos formulários; apresentação combinada montada dinamicamente; textos de erro de duplicidade revisados.
...
- **Security:** sem mudança de policy prevista (nova coluna não é security-relevant); RLS existente permanece como fonte de autorização [referencias.md](Specs-Archive-Implemented-Current-Database-Referencias), [security-model.md](Specs-Archive-Implemented-Current-Security-Security-Model).
