...
- REF-0004 (2026-09-03): o gate agora DELETA o comentário de falha residual (`<!-- sync:release-gate -->`) quando passa — `clearGateFailure` via `DELETE` (novo `GitHubClient.deleteComment`); novo `scripts/spec-github/pre-release-check.js` (checagem preventiva local do corpo do PR antes do push — formato da tabela §23, Spec existe, frontmatter `Issue:` bate); CONVENTIONS §18.9 com exemplo válido da tabela §23 + template `.github/release.md`. `[OBSERVED: git history, PR da REF-0004]`
...
- [ADR-0012: Spec-Driven GitHub Operations](Specs-Decisions-ADR-0012-Spec-Driven-Github-Operations) (itens 8 e 12 revisados)
- [REF-0003: Fluxos automáticos determinísticos sem IA](Specs-Proposed-Refactors-REF-0003-Fluxos-Automaticos-Deterministicos)
