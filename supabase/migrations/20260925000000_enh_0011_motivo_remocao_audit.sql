...
-- Persiste o campo `motivo` em referencia_eventos.detalhes para os eventos
-- de remoção gerados por aplicar_sync_referencias:
--   Seção C (plano): motivo = v_motivo ('ausencia' | 'substituicao')
--   Seção D (sweep retroativo): motivo = 'sweep'
-- Sem mudança de schema (detalhes já é JSONB). Eventos históricos sem motivo
-- coexistem; leitores devem tratar motivo como campo opcional.
-- Autorizado: Lucas Martins Menezes, 2026-09-25
-- ==========================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- ENH-0011: persiste motivo do plano em detalhes para rastreabilidade
    v_identidade := v_identidade || jsonb_build_object('motivo', v_motivo);
  ...
  --    Bug E (20260924010000): exclui refs que já têm evento neste sync_id
  --    para não reprocessar refs recém-arquivadas pela seção C acima.
  FOR v_sweep_id IN
    SELECT r.id
  ...
  LOOP
    ...
    -- ENH-0011: sweep tem motivo próprio
    v_identidade := v_identidade || jsonb_build_object('motivo', 'sweep');
