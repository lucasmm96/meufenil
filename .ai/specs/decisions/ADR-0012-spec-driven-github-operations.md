...
2. **Ligação Spec ↔ Issue 1:1:** campo `Issue: #N` no frontmatter da Spec; label de categoria `spec:<tipo>` no Issue (conjunto fechado: `spec:feat`/`spec:enh`/`spec:ref`/`spec:debt`/`spec:sec`/`spec:test`; dedup por título `[SPEC-ID]`); bloco `SPEC-PROJECTION:START/END` no corpo do Issue (única região editada por Claude; discussão humana fora do bloco nunca é sobrescrita). Todo item real de `proposed/` possui Issue canônica.
