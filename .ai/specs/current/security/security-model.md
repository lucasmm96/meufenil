...
**Última verificação:** 2026-09-25 (ENH-0009 — `decidir_pendencia_referencia` e `reverter_sync_referencias` eliminadas; curadoria/rollback removidos. Antes: 2026-09-11 FEAT-0017 M6)
...
- **Três formas de checagem de admin coexistem** `[CONFIRMED: database, migration]`:
  1. `public.is_admin_user(auth.uid())` — verifica `usuarios.role = 'admin'` (usada em policies de `usuarios`/`background_job_executions` e nos 2 RPCs: `ativar_referencia`, `remover_ou_desativar_referencia`; ENH-0009 removeu `decidir_pendencia_referencia`);
  2. `auth.jwt() ->> 'role' = 'admin'` — claim do JWT Supabase (usada nas policies UPDATE/DELETE de `referencias`);
  3. `public.pode_operar_recuperacao(auth.uid())` — FEAT-0017 M5 (migration 20260906010000, dev 2026-09-06): verifica `role = 'admin'` **E** `usuarios.pode_recuperacao = true` (flag default `false`, concedida manualmente — ver [../database/usuarios.md](../database/usuarios.md)) — guarda exclusiva da RPC de recuperação `restaurar_referencias_de_backup` (ENH-0009 removeu `reverter_sync_referencias`); nenhuma policy a usa.
...
- `aplicar_sync_referencias` (FEAT-0017 M4, reescrita ENH-0009 — migration 20260923000000): SECURITY DEFINER com `search_path = public`; proteção efetiva é a **guarda interna** `auth.role() = 'service_role'` (nenhum cliente — nem admin com sessão — pode aplicar plano). Linhas criadas têm `criado_por` = **ator Sistema** (`sistema@meufenil.local` — fail-high: plano com criações e ator ausente → exceção). ENH-0009 removeu `decidir_pendencia_referencia` (curadoria) e seu GUC `app.audit_origin='curadoria'` `[CONFIRMED: migration 20260923000000, database]`.
- ~~`decidir_pendencia_referencia`~~ — **ELIMINADA (ENH-0009, migration 20260923000000)**.
- ~~`reverter_sync_referencias`~~ — **ELIMINADA (ENH-0009, migration 20260923000000)**.
...
| ~~`decidir_pendencia_referencia`~~ | — | — | — | — | — | **ELIMINADA (ENH-0009)** | ../database/rpc.md |
| ~~`reverter_sync_referencias`~~ | — | — | — | — | — | **ELIMINADA (ENH-0009)** | ../database/rpc.md |
| `restaurar_referencias_de_backup` | Sim | `public` | postgres | postgres | bypassado (efeito no catálogo) | UI do Admin M6 — `src/react-app/services/referencias-sync.service.ts:574` (ação humana, nunca automatizada; ENH-0009 reescrita) | ../database/rpc.md |
