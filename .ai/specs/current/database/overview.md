...
Este diretório documenta o estado REAL do banco de dados PostgreSQL (Supabase) do MeuFenil — tabelas, funções, triggers e políticas RLS — conforme observado nos bancos development e production, complementado pelas migrations versionadas e pelo código que interage com o banco. Desde 2026-09-04, dev e prod DIVERGEM: dev recebeu a ENH-0004 (coluna `marca`, identidade imutável de referências, ordem física das colunas — 2026-09-04) e o FEAT-0017 M1/M4 (tabelas de sincronização, auditoria de `is_ativa`, RPCs de aplicação/curadoria — 2026-09-06); prod segue no schema pré-ENH-0004 até a release (migrations novas em branch de trabalho; promoção segue gate de release) `[INFERRED: migrations não aplicadas em prod; release ainda não criada]`. As seções abaixo descrevem o estado dev pós-FEAT-0017 M1/M4 quando não indicado.
...
| `rpc.md` | As 12 funções do schema `public` (T2) |
...
`[CONFIRMED: database — pg_tables dev 2026-09-06]`. Sem página própria até a promoção da FEAT-0017 (M7 — segue em proposed/); especificadas na própria FEAT-0017, usadas pela rota `api/referencias-sync.ts` (M2–M4) e documentadas em RLS/policies no [security-model](../security/security-model.md) §11.

| Tabela | Propósito curto | RLS | DDL versionado? |
|---|---|---|---|
| `referencia_syncs` | Execuções da sincronização (status, modo, contadores, `alteracoes` jsonb) | Sim | Sim (20260905000000) |
| `referencia_sync_pendencias` | Divergências abertas para curadoria (decisão humana por admin) | Sim | Sim (20260905000000) |
| `referencia_eventos` | Auditoria de eventos da sync e de mudanças manuais de `is_ativa` | Sim | Sim (20260905000000; trigger de auditoria em 20260905010000) |
| `referencia_snapshots` | Snapshots da extração (uma por execução) | Sim | Sim (20260905000000) |
| `referencia_backups` | Backups pré-arquivamento (base do rollback/restauração — M5) | Sim | Sim (20260905000000) |
...
- **Divergência adicional a partir de 2026-09-06 (FEAT-0017 M1/M4)**: migrations 20260905000000/010000/020000 (M1) e 20260906000000 (M4) aplicadas em DEV; prod segue pré-ENH-0004 até a release `[INFERRED: migrations novas em branch de trabalho; promoção segue gate de release]`. Em dev pós-FEAT-0017 M1/M4 `[CONFIRMED: database — catálogo dev 2026-09-06]`: **12 tabelas** (7 + 5 de sync), **100 colunas** (52 + 48), **36 constraints** (`pg_constraint`: 12 PK + 19 FK + 1 UNIQUE + 4 CHECK — medição igual à histórica), **36 índices** (14 + 22), **36 políticas** (31 + 5 `admin_select_*`), **12 funções** (8 + 2 de trigger do M1 + 2 RPCs do M4), **5 enums** (1 legado + 4 de sync) e **3 triggers em `public`** (1 + 2 do M1) + 1 em `auth.users` (inalterado). Prod permanece com 7 tabelas, 52 colunas, 19 constraints, 15 índices, 31 políticas, 10 funções, 1 enum e 3 triggers em `public` até a aplicação.
...
- E1 — Inventário de tabelas, colunas (52), constraints (19), índices (15 em 2026-08-14; dev pós-ENH-0004 = 14), flags RLS, políticas (31), funções (10 em 2026-08-14; dev pós-ENH-0004 = 8), enums (1 em `public`) e triggers (3 em `public` + 1 em `auth.users` em 2026-08-14; dev pós-ENH-0004 = 1 + 1): catálogo dos bancos dev e prod via queries somente-leitura (2026-08-14) + dev após migrations ENH-0004 (2026-09-04) + dev pós-FEAT-0017 M1/M4 (2026-09-06 — 12 tabelas, 100 colunas, 36 constraints, 36 índices, 36 políticas, 12 funções, 5 enums, 3 triggers em `public` + 1 em `auth.users`) `[CONFIRMED: database, migration]`.
- E2 — Conteúdo das migrations em `supabase/migrations/` (baseline, 20260807, 20260810, 20260811, 20260814, 20260815, 20260904000000→040000, FEAT-0017: 20260905000000/010000/020000 e 20260906000000) `[CONFIRMED: migration]`.
