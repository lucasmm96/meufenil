...
**Última verificação:** 2026-09-25 (ENH-0011 — migration 20260925000000: `motivo` adicionado a `detalhes` dos eventos de remoção — seção C: v_motivo ('ausencia'|'substituicao'); seção D/sweep: 'sweep'. Antes: ENH-0009 — migration 20260923000000 reescreveu; corretivas 20260924000000/20260924010000 — definição final em 20260924010000)
**Definição em:** `20260906000000_referencias_sync_aplicacao_curadoria.sql` (FEAT-0017 M4, original); **reescrita** por `20260923000000_enh_0009_delecao_fisica_sync_simplificacao.sql` (ENH-0009); **corrigida** por `20260924000000_enh_0009_correcao_aplicar_sync.sql` (bugs A–D) e `20260924010000_enh_0009_correcao2_sweep_mensagem.sql` (bugs E–F); **atualizada** por `20260925000000_enh_0011_motivo_remocao_audit.sql` (ENH-0011) `[CONFIRMED: migration]`
...
- **Efeitos ENH-0009/ENH-0011 (transação única — qualquer exceção desfaz tudo):** (1) ator Sistema resolvido somente se o plano cria linhas (fail-high); (2) `criacoes[]` (`op='create'`) → INSERT em `referencias` (`is_global=true`, `is_ativa=true`, `criado_por`=Sistema); (3) `arquivamentos[]` (`op='archive'`, `referencia_id`, `motivo:"ausencia"|"substituicao"`) → tentativa de **DELETE físico** (ausência: sem relacionamentos; substituição: sempre soft-archive) com fallback `UPDATE is_ativa=false` em caso de FK violation; evento `referencia_deletada` (deleção bem-sucedida) ou `referencia_arquivada` (arquivamento); **ENH-0011:** `detalhes` dos eventos de remoção da seção C = `{nome, marca, fenil_mg_por_100g, motivo}` (motivo = `v_motivo`); (4) sweep retroativo: DELETE de até 100 globais `is_ativa=false` sem relacionamentos por execução; **ENH-0011:** `detalhes` do evento sweep = `{nome, marca, fenil_mg_por_100g, motivo:'sweep'}`; (5) UPDATE da sync: `equivalentes`, `criadas`, `arquivadas`, `deletadas`. `alteracoes` sempre `[]` (rollback removido). Nunca reativa; nunca decide pendências (removidas)
- **Erros e edge cases:** identidade já ativa → `23505` `'Estado mudou durante a sync: identidade já ativa — % % %'` (nome/marca/fenil) → ROLLBACK total; arquivamento de inativa → `'Estado mudou durante a sync: referência % não encontrada ou já inativa'` → ROLLBACK total; op desconhecida → `'Operação desconhecida no plano: %'`; sync não-running; plano inválido. Retorna `{sync_id, equivalentes, criadas, arquivadas, deletadas}` — a rota sempre conclui `success` (sem `divergencias`)
- **Chamadores no código:** `api/referencias-sync.ts` estágio 7 (service role) `[CONFIRMED: code]`
- **Testes:** `src/shared/security/rpc-referencias-sync.test.ts` (REAL, ENH-0009/ENH-0011 — 10 testes): plano manual com criação + arquivamento (motivo `substituicao`; **ENH-0011:** verifica `detalhes.motivo='substituicao'` e sweep com `detalhes.motivo='sweep'`); motor real; plano vazio sem Sistema; negativos: versão inválida, sync inexistente/não-running, op desconhecida, arquivamento de inativa → ROLLBACK, `23505` → ROLLBACK; authenticated (admin) → permissão negada; **ENH-0011:** novo teste `ausencia` verifica `detalhes.motivo='ausencia'` no evento `referencia_deletada` `[CONFIRMED: test]`
- **Evidências:** E1 — definição no banco dev = migration 20260924010000 (corretiva final; original: 20260923000000; 20260924000000 corrigiu bugs A–D) `[CONFIRMED: database, migration]`
