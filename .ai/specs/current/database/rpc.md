...
- **Chamadores no código:** nenhum ainda — ação humana excepcional (admin + `pode_recuperacao`); exposição em UI é escopo de milestone posterior `[CONFIRMED: code — ausência de chamador; RPC e testes no M5]`
...
- **Chamadores no código:** nenhum ainda — ação humana excepcional (admin + `pode_recuperacao`); UI em milestone posterior `[CONFIRMED: code — ausência de chamador; RPC e testes no M5]`
...
- E3 — Chamadores: `grep` de `.rpc(` em `src/`, `api/`, `supabase/functions/` (2026-08-13 — 3 chamadas legadas; 2026-09-06 — +1: `aplicar_sync_referencias` em `api/referencias-sync.ts` estágio 7; `decidir_pendencia_referencia`, `reverter_sync_referencias` e `restaurar_referencias_de_backup` sem chamador no código — curadoria chega no M6; recuperação é ação humana excepcional, nunca automatizada — ver seções) `[CONFIRMED: code]`
