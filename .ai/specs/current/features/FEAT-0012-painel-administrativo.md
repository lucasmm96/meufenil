...
3. `useBackgroundJobsAdmin` carrega overview + execuções (filtros job/status/período com reset de página; paginação server-side; tabela desktop × cards mobile; linha clicável abre painel de detalhes com Run ID, tempos e `details` em `<pre>`) `[CONFIRMED: code — Admin.tsx:319-592]`.
